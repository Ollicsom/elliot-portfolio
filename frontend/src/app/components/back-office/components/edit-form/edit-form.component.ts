...
export class EditFormComponent implements OnInit, OnDestroy {
  ...
  changeSeriePicture(event: any) {
    if(event.target.files){
        ...
        this.apiService.uploadImage(photoData).subscribe((file) => {
            this.serieForm.get('main_photo_file').setValue(file.filename)
        });
    }
  }
  ...
  changePhotoPicture(event: any, index: number) {
    if(event.target.files){
        ...
        photoData.append('upload', event.target.files[0], event.target.files[0].name);
        this.apiService.uploadImage(photoData).subscribe((file) => {
            this.serieForm.get('Photos').get('' + index).get('fileName').setValue(file.filename)
        });
    }
  }
  ...
  async savePhoto() {
    await this.apiService.updateOrCreateSerie(this.serieForm.value).subscribe(serie => {
      ...
      this.toastsService.showToast('Succès', 'Les modifications ont bien été prisent en compte', 'sucess');
    },
